...
var (imgKey, subKey) = await WbiSign.GetKeys();

var signedParams = await WbiSign.Encrypt(query, imgKey, subKey);

var queryString = await WbiSign.CreateQueryString(signedParams);
...
internal static class WbiSign {
	...
	public static async Task<(string, string)> GetKeys() {
		Initialize();
	...
	public static async Task<Dictionary<string, string>> Encrypt(Dictionary<string, string> parameters, string imgKey, string subKey) {
		...
		var query = await CreateQueryString(parameters); // 序列化参数
	...
	public static async Task<string> CreateQueryString(Dictionary<string, string> parameters) {
